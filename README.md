...
```sh
docker buildx build --platform linux/arm64,linux/amd64 -t nnzbz/vertx:4.3.7 --build-arg VERSION=4.3.7 --build-arg JDK_VERSION=18 . --push
docker buildx build --platform linux/arm64,linux/amd64 -t nnzbz/vertx:4.3.7-alpine --build-arg VERSION=4.3.7 --build-arg JDK_VERSION=alpine . --push
# latest
docker buildx build --platform linux/arm64,linux/amd64 -t nnzbz/vertx:latest --build-arg VERSION=4.3.7 --build-arg JDK_VERSION=18 . --push
docker buildx build --platform linux/arm64,linux/amd64 -t nnzbz/vertx:alpine --build-arg VERSION=4.3.7 --build-arg JDK_VERSION=alpine . --push
```
...
docker run -d --init --restart=always \
  ...
  -e PROG_ARGS=run xxx.xxx.verticle.MainVerticle -cp conf/*:lib/*.jar -ha -hagroup xxx -Dhazelcast.logging.type=slf4j --launcher-class=io.vertx.core.Launcher \
...
services:
  svr:
    image: nnzbz/vertx:4.3.7
    init: true
    environment:
      - PROG_ARGS=run xxx.xxx.verticle.MainVerticle -cp conf/*:lib/*.jar -ha -hagroup xxx -Dhazelcast.logging.type=slf4j --launcher-class=io.vertx.core.Launcher
